...
type CandidateSkillHandler struct {
	service interfaces.CandidateSkillService
}

// NewCandidateSkillHandler creates a new candidate skill handler
func NewCandidateSkillHandler(service interfaces.CandidateSkillService) *CandidateSkillHandler {
	return &CandidateSkillHandler{service: service}
}
...
// GetCandidateSkillsByUserID handles GET /users/{userId}/skills request
func (h *CandidateSkillHandler) GetCandidateSkillsByUserID(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	userID := chi.URLParam(r, "userId")

	candidateSkills, err := h.service.GetCandidateSkillsByUserID(ctx, userID)
	if err != nil {
		http.Error(w, "Failed to retrieve candidate skills", http.StatusInternalServerError)
		return
	}
