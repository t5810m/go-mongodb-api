...
type JobCategoryHandler struct {
	service interfaces.JobCategoryService
}

// NewJobCategoryHandler creates a new job category handler
func NewJobCategoryHandler(service interfaces.JobCategoryService) *JobCategoryHandler {
...
// UpdateJobCategory handles PUT /jobcategories/{id} request
func (h *JobCategoryHandler) UpdateJobCategory(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	jobCategoryID := chi.URLParam(r, "id")

	var jobCategory models.JobCategory
	if err := json.NewDecoder(r.Body).Decode(&jobCategory); err != nil {
		http.Error(w, "Invalid request body", http.StatusBadRequest)
		return
	}
	...
	updated, err := h.service.UpdateJobCategory(ctx, jobCategoryID, &jobCategory)
	if err != nil {
		http.Error(w, "Job category not found", http.StatusNotFound)
		return
	}
