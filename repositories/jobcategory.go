...
// Update updates allowed fields of a job category and returns the updated document
func (r *JobCategoryRepository) Update(ctx context.Context, id string, jobCategory *models.JobCategory) (*models.JobCategory, error) {
	objID, err := bson.ObjectIDFromHex(id)
	if err != nil {
		return nil, err
	}
	...
	update := bson.M{
		"$set": bson.M{
			...
			"updated_time": jobCategory.UpdatedTime,
			"updated_by":   jobCategory.UpdatedBy,
		},
	}

	opts := options.FindOneAndUpdate().SetReturnDocument(options.After)
	var updated models.JobCategory
