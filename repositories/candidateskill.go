...
func (r *CandidateSkillRepository) GetAll(ctx context.Context, page, limit int, filters map[string]string, sort, order string) ([]models.CandidateSkill, int64, error) {
	...
	if userID, exists := filters["user_id"]; exists && userID != "" {
	...
	sortableFields := []string{"user_id", "skill_id", "proficiency_level", "created_time"}
...
// GetByUserID retrieves all skills for a specific user
func (r *CandidateSkillRepository) GetByUserID(ctx context.Context, userID string) ([]models.CandidateSkill, error) {
	objID, err := bson.ObjectIDFromHex(userID)
	if err != nil {
		return nil, err
	}
