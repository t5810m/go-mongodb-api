...
func (r *ApplicationRepository) GetAll(ctx context.Context, page, limit int, filters map[string]string, sort, order string) ([]models.Application, int64, error) {
	...
	sortableFields := []string{"status", "job_id", "user_id", "applied_time"}
...
// GetByUserID retrieves all applications from a specific user
func (r *ApplicationRepository) GetByUserID(ctx context.Context, userID string) ([]models.Application, error) {
	objID, err := bson.ObjectIDFromHex(userID)
	if err != nil {
		return nil, err
	}
